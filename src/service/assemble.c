...
static void bmi088_spi_txrx_complete_callback(SPI_HandleTypeDef* hspi);
static void bmi088_spi_error_forward_callback(SPI_HandleTypeDef* hspi);
...
static void assemble_imu(void) {
    ...
    assert(bmi088_make_config(&bmi088_config, &bmi088_ops) == IMU_STATUS_OK);
    bmi088_config.accel_int_pin = ACC_INT_Pin;
