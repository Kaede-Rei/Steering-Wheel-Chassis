...
typedef FDCAN_HandleTypeDef BspCanHandle;
typedef FDCAN_RxHeaderTypeDef BspCanRxHeader;
...
typedef void (*BspCanRxCallback)(BspCanHandle* hcan,
...
extern const struct BspCanInterface {
    ...
    BspCanStatus (*start)(BspCanHandle* hcan);
    BspCanStatus (*send)(BspCanHandle* hcan, uint32_t id, const uint8_t* data, uint8_t len);
    BspCanStatus (*register_rx_callback)(BspCanHandle* hcan, BspCanRxCallback callback, void* user);
